...
import '../services/analysis_unsettled_recent_buy_rest_api_service.dart';
import '../widgets/unsettled_recent_buys_csv_dialog.dart';

class AnalysisUnsettledRecentBuy extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisUnsettledRecentBuy(
      {super.key,
      required this.userSelectedAnalysis,
      required this.userSelectedStock});

  @override
  State<AnalysisUnsettledRecentBuy> createState() =>
      _AnalysisUnsettledRecentBuyState();
}

class _AnalysisUnsettledRecentBuyState
    extends State<AnalysisUnsettledRecentBuy> {
  AnalysisType? _selectedAnalysisType;
  // unsettledBuyModel.AnalyticsUnsettledClosestBuyTableResponse? unsettledBuy;
  AnalyticsUnsettledRecentBuyResponse? unsettledRecentBuyDataNew;
  ...
  Future<void> getUnsettledBuysDataNew() async {
    ...
    AnalysisUnsettledRecentBuyRestApiService()
        .analyticsUnsettledRecentBuyTableNew(
        AnalyticsUnsettledRecentBuyTableRequest(
        ...
        .then((value) {
      if (value!.status!) {
        unsettledRecentBuyDataNew = value;
        _updateState();
      } else {
  ...
  void sortUnsettledClosestBuys(int titleIndex, bool increasingOrder) {
    if (unsettledRecentBuyDataNew != null && unsettledRecentBuyDataNew!.data!.isNotEmpty) {
      switch (titleIndex) {
        case 0:
          {
            if (increasingOrder) {
              unsettledRecentBuyDataNew!.data!
        ...
        case 1:
          {
            if (increasingOrder) {
              unsettledRecentBuyDataNew!.data!.sort((a, b) =>
    ...
    return Container(
      child: Column(
        ...
        children: [
            ...
            child: TextButton(
              onPressed: () {
                showDialog(
                    context: context,
                    builder: (context) {
                      return UnsettledRecentBuysCsvDialog(
                          stockName: _selectedStock);
                    });
              },
              child: Text(
          ...
          unsettledRecentBuyDataNew!.data!.isNotEmpty
              ? Container(
          ...
          Row(
            ...
            children: [
              Text('Net Cash Gain'),
              Text(
                '${double.parse(unsettledRecentBuyDataNew!.total!.netCashGain!.toString()) > 0 ? "+" : ""} ${amountToInrFormat(context, double.parse(unsettledRecentBuyDataNew!.total!.netCashGain!.toString()))}',
                style: TextStyle(
                  color: double.parse(unsettledRecentBuyDataNew!.total!.netCashGain!.toString()) > 0
                      ? Colors.green
          ...
          Row(
            ...
            children: [
              ...
              Text("${unsettledRecentBuyDataNew!.total!.netStockBought} / ${unsettledRecentBuyDataNew!.total!.netStockSold}")
              // Text(
              //     "${amountToInrFormat(context, double.parse(unsettledRecentBuyDataNew!.total!.netStockBought!.toString()))} / ${amountToInrFormat(context, double.parse(unsettledRecentBuyDataNew!.total!.netStockSold!.toString()))!.replaceAll("-", "")}"),
            ],
          ),
          SizedBox(
            height: 7,
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _selectedAnalysisType!.itemId == 11 //2
                          ? unsettledRecentBuyDataNew != null
