...
class HiddenOrdersTab extends StatefulWidget {
  final String? selectedTicker;
  const HiddenOrdersTab({super.key, this.selectedTicker});

  @override
  State<HiddenOrdersTab> createState() => _HiddenOrdersTabState();
}

class _HiddenOrdersTabState extends State<HiddenOrdersTab> {
                      ...
                      child: ElevatedButton(
                          onPressed: () async {
                            ...
                            if(_orderProvider!.limitPrice == 'null' || _orderProvider!.limitPrice == "") {
                              _orderProvider!.limitPriceErrorText = "Please enter min limit price";
                            } else {
  ...
  Future<void> callInitialApi() async {
    try {
      await _orderProvider!.fetchHiddenOrders(_currencyConstantsProvider!);
        ...
        child: Container(
          ...
          child: Column(
            children: [
                            ...
                            if (orderData.orderType == "BUY")
                              Expanded(
                                child: Text(
                                  ...
                                  "${orderData.orderStockName?.trim()} :Lad-${orderData.orderLadName} ${orderData.orderUnits}UNITS",
                                  style: TextStyle(
                                    fontSize: 16,
                                    color: (themeProvider.defaultTheme)?Colors.black:Colors.white,
              ...
              Visibility(
                visible: orderState.hiddenOrderActionVisible[i - 1],
                child: Row(
                  children: [
                    Expanded(
                        child: ElevatedButton(
                    ...
                    Expanded(
                        child: ElevatedButton(
                            ...
                            onPressed: () async {
                              ...
                              //       context,
                              //       "Warning",
                              //       "Partial Sell at this price is not possible as the current price is higher than the last trade executed price.",
                              //     );
                              //   }
                              // } else {
                              //   entryPrice = orderData.orderOpenPrice ?? 0.0;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<OrderProvider>(builder: (_, state, __) {
      return state.isHiddenOrdersLoading
          ? Center(child: Text("N/A"))
          : state.hiddenOrders!.data!.isEmpty
          ? Center(
        child: Text(
          "No open orders",
        ...
        itemBuilder: (context, index) {
          return orderUI(
            context: context,
            orderData: state.hiddenOrders!.data![index],
            i: index + 1,
            orderState: state,
          );
        },
        itemCount: state.hiddenOrders!.data!.length,
      );
    });
  }
}
