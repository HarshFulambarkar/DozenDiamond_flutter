...
import '../models/trade_table_csv_request.dart';
import '../models/trade_table_csv_response.dart';
import '../models/trade_table_request.dart';
import '../models/trade_table_response.dart';

class TradeTableRestApiService {
  ...
  Future<TradeTableResponse> tradeTable(
      TradeTableRequest stockData,
    ...
    try {
      ...
      var url = Uri.parse(
          "$baseUrl_v2$userId/analytics/all-trade");
  ...
  Future<TradeCsvTableResponse> tradeTableCsvRequest(
      TradeTableCsvRequest stockName) async {
    try {
      // var accessToken = await _generateToken;
      String userAccessToken = (await SharedPreferenceManager.getUserAccessToken()) ?? "";
      ...
      var url = Uri.parse(
          "$baseUrl_v2$userId/analytics/all-trade/downloadCsv");
