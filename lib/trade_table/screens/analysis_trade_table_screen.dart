...
import '../models/trade_table_request.dart';
import '../models/trade_table_response.dart';
import '../services/trade_table_rest_api_service.dart';
import '../widgets/trade_table_csv_dialog.dart';

class AnalysisTradeTableScreen extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisTradeTableScreen(
      {super.key,
        required this.userSelectedAnalysis,
        required this.userSelectedStock});

  @override
  State<AnalysisTradeTableScreen> createState() =>
      _AnalysisTradeTableScreenState();
}

class _AnalysisTradeTableScreenState
    extends State<AnalysisTradeTableScreen> {
  AnalysisType? _selectedAnalysisType;
  // unsettledBuyModel.AnalyticsUnsettledClosestBuyTableResponse? unsettledBuy;
  TradeTableResponse? tradeTableData;
  ...
  void sortUnsettledClosestBuys(int titleIndex, bool increasingOrder) {
    if (tradeTableData != null && tradeTableData!.data!.isNotEmpty) {
      switch (titleIndex) {
        case 0:
          {
            if (increasingOrder) {
    ...
    return Container(
      child: Column(
        ...
        children: [
          ...
          tradeTableData!.data!.isNotEmpty
              ? Container(
          ...
          Row(
            ...
            children: [
              Text('Net Cash Gain'),
              Text(
                '${double.parse(tradeTableData!.total!.netCashGain!.toString()) > 0 ? "+" : ""} ${amountToInrFormat(context, double.parse(tradeTableData!.total!.netCashGain!.toString()))}',
                style: TextStyle(
                  color: double.parse(tradeTableData!.total!.netCashGain!.toString()) > 0
                      ? Colors.green
          ...
          Row(
            ...
            children: [
              ...
              Text("${tradeTableData!.total!.netStockBought} / ${tradeTableData!.total!.netStockSold}")
              // Text(
              //     "${amountToInrFormat(context, double.parse(tradeTableData!.total!.netStockBought!.toString()))} / ${amountToInrFormat(context, double.parse(tradeTableData!.total!.netStockSold!.toString()))!.replaceAll("-", "")}"),
            ],
          ),
          SizedBox(
            height: 7,
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _selectedAnalysisType!.itemId == 13 //2
                          ? tradeTableData != null
