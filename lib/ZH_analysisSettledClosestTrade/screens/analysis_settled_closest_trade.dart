...
import '../widgets/settled_closest_trade_csv_dialog.dart';

class AnalysisSettledClosestTrade extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisSettledClosestTrade(
      {super.key,
      required this.userSelectedAnalysis,
      required this.userSelectedStock});

  @override
  State<AnalysisSettledClosestTrade> createState() =>
      _AnalysisSettledClosestTradeState();
}

class _AnalysisSettledClosestTradeState
    extends State<AnalysisSettledClosestTrade> {
  AnalysisType? _selectedAnalysisType;
  SettledClosestTradeResponse? settledBuyDataNew;
  ...
  void sortUnsettledClosestBuys(int titleIndex, bool increasingOrder) {
    if (settledBuyDataNew != null && settledBuyDataNew!.data!.isNotEmpty) {
      switch (titleIndex) {
        case 0:
          {
            if (increasingOrder) {
              settledBuyDataNew!.data!
        ...
        case 1:
          {
            if (increasingOrder) {
              settledBuyDataNew!.data!.sort((a, b) =>
    ...
    return Container(
      child: Column(
        ...
        children: [
            ...
            child: TextButton(
              onPressed: () {
                showDialog(
                    context: context,
                    builder: (context) {
                      return SettledClosestTradeCsvDialog(
                          stockName: _selectedStock);
                    });
              },
              child: Text(
          ...
          settledBuyDataNew!.data!.isNotEmpty
              ? Container(
          ...
          Row(
            ...
            children: [
              Text('Net Cash Gain'),
              Text(
                '${double.parse(settledBuyDataNew!.total!.netCashGain!.toString()) > 0 ? "+" : ""} ${amountToInrFormat(context, double.parse(settledBuyDataNew!.total!.netCashGain!.toString()))}',
                style: TextStyle(
                  color: double.parse(settledBuyDataNew!.total!.netCashGain!.toString()) > 0
                      ? Colors.green
          ...
          Row(
            ...
            children: [
              ...
              Text("${settledBuyDataNew!.total!.netStockBought} / ${settledBuyDataNew!.total!.netStockSold}")
              // Text(
              //     "${amountToInrFormat(context, double.parse(settledBuyDataNew!.total!.netStockBought!.toString()))} / ${amountToInrFormat(context, double.parse(settledBuyDataNew!.total!.netStockSold!.toString()))!.replaceAll("-", "")}"),
            ],
          ),
          SizedBox(
            height: 7,
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _selectedAnalysisType!.itemId == 10 //2
                          ? settledBuyDataNew != null
