...
class SharekhanRestApiService {
  ...
  Future<SharekhanLoginResponse> doSharekhanLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/sharekhan/sharekhanCustomerLogin");
      ...
      print("below is Sharekhan  login response");
      print(url);
      print(payload);
      print(response.body);
      if (httpStatusChecker(response)) {
        return SharekhanLoginResponse.fromJson(jsonDecode(response.body));
      } else {
