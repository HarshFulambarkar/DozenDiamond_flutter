...
class ShoonyaRestApiService {
  ...
  Future<ShoonyaLoginResponse> doShoonyaLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/shoonyaCustomerLogin");
      ...
      if (httpStatusChecker(response)) {
        return ShoonyaLoginResponse.fromJson(jsonDecode(response.body));
      } else {
