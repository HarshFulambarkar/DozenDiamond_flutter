...
class ZerodhaRestApiService {
  ...
  Future<ZerodhaLoginResponse> doZerodhaLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/zerodhaCustomerLogin");
      ...
      print("below is zerodha neo login response");
      print(url);
      print(payload);
      print(response.body);
      if (httpStatusChecker(response)) {
        return ZerodhaLoginResponse.fromJson(jsonDecode(response.body));
      } else {
