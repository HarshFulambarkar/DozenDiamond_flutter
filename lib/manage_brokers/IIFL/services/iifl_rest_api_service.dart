...
class IiflRestApiService {
  ...
  Future<IiflLoginResponse> doIiflLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/iifl/login");
      ...
      print("below is iifl login response");
      print(url);
      print(payload);
      print(response.body);
      if (httpStatusChecker(response)) {
        return IiflLoginResponse.fromJson(jsonDecode(response.body));
      } else {
