...
class ProfitmartRestApiService {
  ...
  Future<ProfitmartLoginResponse> doProfitmartLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/profitmartCustomerLogin");
      ...
      if (httpStatusChecker(response)) {
        return ProfitmartLoginResponse.fromJson(jsonDecode(response.body));
      } else {
