...
class PaachPaisaRestApiService {
...
Future<PaachPaiseCustomerRequestTokenResponse> sendPaachPaisaOtp(
    Map<String, dynamic>? requestBody) async {
  try {
    var accessToken = await _generateToken;

    final userId = await getUserID();
    var url = Uri.parse("$baseUrl_v2$userId/5paisaCustomerRequestToken");
    ...
    print("below is paach paisa login response");
    print(url);
    print(payload);
    print(response.body);
    if (httpStatusChecker(response)) {
      return PaachPaiseCustomerRequestTokenResponse.fromJson(jsonDecode(response.body));
    } else {
...
//
Future<PaachPaiseCustomerAccessTokenResponse> doPaachPaisaLogin(
    Map<String, dynamic>? requestBody) async {
  try {
    var accessToken = await _generateToken;

    final userId = await getUserID();
    var url = Uri.parse("$baseUrl_v2$userId/5paisaCustomerAccessToken");
    var payload = json.encode(requestBody);
    var response = await http.post(url, body: payload, headers: {
      "Content-Type": "application/json",
      ApiConstant.authorizationHeaderKeyName: accessToken,
    ...
    print(response.body);
    if (httpStatusChecker(response)) {
      return PaachPaiseCustomerAccessTokenResponse.fromJson(jsonDecode(response.body));
    } else {
      throw HttpApiException(errorCode: 404);
    }
  } catch (e) {
