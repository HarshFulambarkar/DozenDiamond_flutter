...
class AngleOneRestApiService {
  ...
  Future<AngleOneLoginResponse> doAngleOneLogin(
    ...
    try {
      ...
      var url = Uri.parse("$baseUrl_v2$userId/angelOneCustomerLogin");
  ...
  Future<AngleOneLogoutResponse> doAngleOneLogout(
      Map<String, dynamic>? requestBody) async {
    // String tempbaseUrl_v2 = "https://2fw8x25q-3000.inc1.devtunnels.ms/api/v1/user/";
    try {
      var accessToken = await _generateToken;
      ...
      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/angelOneCustomerLogout");
      // var url = Uri.parse("$tempbaseUrl_v2$userId/smcTradeCustomerAccessToken");
      var payload = json.encode(requestBody);
      var response = await http.post(url, body: payload, headers: {
        "Content-Type": "application/json",
      ...
      print(response.body);
      if (httpStatusChecker(response)) {
        return AngleOneLogoutResponse.fromJson(jsonDecode(response.body));
      } else {
        throw HttpApiException(errorCode: 404);
      }
    } catch (e) {
