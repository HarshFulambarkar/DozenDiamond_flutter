...
class MotilalOswalRestApiService {
  ...
  Future<MotilalOswalLoginResponse> motilalOswalLogin(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/motilal-oswal/login");
      ...
      print("below is login response");
      print(url);
      print(payload);
      print(response.body);
      if (httpStatusChecker(response)) {
        return MotilalOswalLoginResponse.fromJson(jsonDecode(response.body));
      } else {
  ...
  Future<MotilalOswalVerifyOtpResponse> motilalOswalVerifyOtp(
      Map<String, dynamic>? requestBody) async {
    try {
      var accessToken = await _generateToken;

      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/motilal-oswal/verfiy-otp");
      var payload = json.encode(requestBody);
      var response = await http.post(url, body: payload, headers: {
        "Content-Type": "application/json",
        ApiConstant.authorizationHeaderKeyName: accessToken,
      ...
      print(response.body);
      if (httpStatusChecker(response)) {
        return MotilalOswalVerifyOtpResponse.fromJson(jsonDecode(response.body));
      } else {
        throw HttpApiException(errorCode: 404);
      }
    } catch (e) {
  ...
  Future<MotilalOswalLogoutResponse> doMotilalOswalLogout(
      Map<String, dynamic>? requestBody) async {
    ...
    try {
      ...
      final userId = await getUserID();
      var url = Uri.parse("$baseUrl_v2$userId/motilal-oswal/logout");
      ...
      print(response.body);
      if (httpStatusChecker(response)) {
        return MotilalOswalLogoutResponse.fromJson(jsonDecode(response.body));
      } else {
        throw HttpApiException(errorCode: 404);
      }
    } catch (e) {
