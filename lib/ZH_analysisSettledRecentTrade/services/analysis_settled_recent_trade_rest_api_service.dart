...
import '../constants/dummy_analysis_settled_recent_trade_data.dart';
import '../models/settled_recent_trade_csv_request.dart';
import '../models/settled_recent_trade_csv_response.dart';
...
class AnalysisSettledRecentTradeRestApiService {
  ...
  Future<SettledRecentTradeTableResponse?> settledRecentTradeTable(
      SettledRecentTradeTableRequest stockData, bool recentSorting) async {
    try {
      ...
      var url = Uri.parse(
          "$baseUrl$userId/stock/analytic-table/settled-trade?sort=recent");
      ...
      if (httpStatusChecker(response)) {
        ...
        return SettledRecentTradeTableResponse().fromJson(apiResponse);
      } else {
        if (kDebugMode || kIsWeb) {
          return DummyAnalysisSettledRecentTradeData()
              .settledRecentTradeTableDummy();
        } else {
  ...
  Future<SettledRecentTradeResponse>
  settledRecentTradeTableNew(
      SettledRecentTradeTableRequest stockData,
    ...
    try {
      ...
      var url = Uri.parse(
          "$baseUrl_v2$userId/analytics/settled-recent-buys");
      ...
      if (httpStatusChecker(response)) {
        dynamic apiResponse = jsonDecode(response.body);
        return SettledRecentTradeResponse
  ...
  Future<SettledRecentTradeCsvResponse> settledRecentTradeCsvRequest(
      SettledRecentTradeCsvRequest stockName) async {
    try {
      // var accessToken = await _generateToken;
      String userAccessToken = (await SharedPreferenceManager.getUserAccessToken()) ?? "";
      ...
      var url = Uri.parse(
          "$baseUrl_v2$userId/analytics/settled-recent-buys/downloadCsv");
      ...
      if (httpStatusChecker(response)) {
        return SettledRecentTradeCsvResponse()
