...
import '../models/settled_recent_trade_response.dart';
import '../models/settled_recent_trade_table_request.dart';
import '../models/settled_recent_trade_table_response.dart'
    as settledTradeModel;
import '../services/analysis_settled_recent_trade_rest_api_service.dart';
import '../widgets/settled_recent_trade_csv_dialog.dart';

class AnalysisSettledRecentTrade extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisSettledRecentTrade(
      {super.key,
      required this.userSelectedAnalysis,
      required this.userSelectedStock});

  @override
  State<AnalysisSettledRecentTrade> createState() =>
      _AnalysisSettledRecentTradeState();
}

class _AnalysisSettledRecentTradeState
    extends State<AnalysisSettledRecentTrade> {
  AnalysisType? _selectedAnalysisType;
  // unsettledBuyModel.AnalyticsUnsettledClosestBuyTableResponse? unsettledBuy;
  SettledRecentTradeResponse? settledRecentBuyDataNew;
  ...
  void initState() {
    ...
    getSettledRecentBuysDataNew();
    // getSelectedStockList();
  }

  Future<void> getSettledRecentBuysDataNew() async {
    loadingTableData = true;
    AnalysisSettledRecentTradeRestApiService()
        .settledRecentTradeTableNew(
        SettledRecentTradeTableRequest(
  ...
  void sortUnsettledClosestBuys(int titleIndex, bool increasingOrder) {
    if (settledRecentBuyDataNew != null && settledRecentBuyDataNew!.data!.isNotEmpty) {
      switch (titleIndex) {
        case 0:
          {
            if (increasingOrder) {
              settledRecentBuyDataNew!.data!
        ...
        case 1:
          {
            if (increasingOrder) {
              settledRecentBuyDataNew!.data!.sort((a, b) =>
    ...
    return Container(
      child: Column(
        ...
        children: [
            ...
            child: TextButton(
              onPressed: () {
                showDialog(
                    context: context,
                    builder: (context) {
                      return SettledRecentTradeCsvDialog(
                          stockName: _selectedStock);
                    });
              },
              child: Text(
          ...
          settledRecentBuyDataNew!.data!.isNotEmpty
              ? Container(
          ...
          Row(
            ...
            children: [
              Text('Net Cash Gain'),
              Text(
                '${double.parse(settledRecentBuyDataNew!.total!.netCashGain!.toString()) > 0 ? "+" : ""} ${amountToInrFormat(context, double.parse(settledRecentBuyDataNew!.total!.netCashGain!.toString()))}',
                style: TextStyle(
                  color: double.parse(settledRecentBuyDataNew!.total!.netCashGain!.toString()) > 0
                      ? Colors.green
          ...
          Row(
            ...
            children: [
              ...
              Text("${settledRecentBuyDataNew!.total!.netStockBought} / ${settledRecentBuyDataNew!.total!.netStockSold}")
              // Text(
              //     "${amountToInrFormat(context, double.parse(settledRecentBuyDataNew!.total!.netStockBought!.toString()))} / ${amountToInrFormat(context, double.parse(settledRecentBuyDataNew!.total!.netStockSold!.toString()))!.replaceAll("-", "")}"),
            ],
          ),
          SizedBox(
            height: 7,
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _selectedAnalysisType!.itemId == 12 //2
                          ? settledRecentBuyDataNew != null
