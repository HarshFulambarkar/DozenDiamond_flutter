...
import '../models/alpha_gain_vs_open_price_request.dart';
import '../models/analysis_alpha_gain_vs_open_price_response.dart';
import '../services/analysis_alpha_gain_vs_open_price_rest_api_service.dart';

class AnalysisAlphaGainVsOpenPriceScreen extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisAlphaGainVsOpenPriceScreen({
    super.key,
  ...
  @override
  State<AnalysisAlphaGainVsOpenPriceScreen> createState() =>
      _AnalysisAlphaGainVsOpenPriceScreenState();
}

class _AnalysisAlphaGainVsOpenPriceScreenState
    extends State<AnalysisAlphaGainVsOpenPriceScreen> {
  ...
  AnalysisAlphaGainVsOpenPriceResponse? analysisAlphaGainVsOpenPriceResponse;
  ...
  Future<void> getAlphaGainVsOpenPrice() async {
    // 100 indicates fetching all records from starting
    isGraphLoading = true;
    try {
      analysisAlphaGainVsOpenPriceResponse =
      await AnalysisAlphaGainVsOpenPriceRestApiService()
          .analyticsAlphaGainVsOpenPriceGraph(
      ...
      if (analysisAlphaGainVsOpenPriceResponse!.status! &&
          analysisAlphaGainVsOpenPriceResponse!.data != null) {
        if (analysisAlphaGainVsOpenPriceResponse!.data!.isNotEmpty) {
          List<FlSpot> stockOwn = [];
          List<FlSpot> alpha5Gain = [];
          List<FlSpot> alpha10Gain = [];
          analysisAlphaGainVsOpenPriceResponse!.data!.forEach((stockData) {
            stockOwn.add(
              FlSpot(
            ...
            alpha5Gain.add(
              FlSpot(
                double.parse(stockData.price!.toString()),
                double.parse(stockData.alpha5Gain!.toStringAsFixed(2)),
              ),
            );

            if (stockData.alpha10Gain != null) {
              alpha10Gain.add(
                FlSpot(
                  double.parse(stockData.price!.toString()),
                  double.parse(stockData.alpha10Gain!.toStringAsFixed(2)),
                ),
              );
            }
          });
        ...
        warningMessage =
            analysisAlphaGainVsOpenPriceResponse!.message ??
  ...
  Widget alphaGainVsOpenOrderPriceCsvDialog() {
    return Container(
      child: TextButton(
        onPressed: () {
          showDialog(
  ...
  Widget alphaGainVsOpenPriceWidget() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.end,
      mainAxisSize: MainAxisSize.min,
      children: [
        alphaGainVsOpenOrderPriceCsvDialog(),
                  ...
                  getTooltipItems: (value) {
                    ...
                    for (int i = 0; i < value.length; i++) {
                      if (i % 2 == 0) {
                            ...
                            children: [
                              ...
                              TextSpan(
                                ...
                                "Price: ${value[i].x}\n${(value[i].bar.color!.value == Colors.purple.value)
                                    ? 'Alpha10 Gain'
                                    : (value[i].bar.color!.value == Colors.orange.value)
                                    ? 'Alpha5 Gain'
                        ...
                        // tooltipData.add(
                        //     LineTooltipItem("Nothing to show", TextStyle(fontSize: 0)));
                      } else {
                        tooltipData.add(
                          LineTooltipItem(
                            "",
                    ...
                    children: [
                      ...
                      _selectedAnalysisType!.itemId ==
                        ...
                        child: Center(
                          ...
                          : (analysisAlphaGainVsOpenPriceResponse!.data ==
                          null)
                          ? Center(
                        child: Container(
            ...
            Expanded(
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  AnalysisAlphaGainVsOpenPriceRestApiService()
                      .alphaGainVsOpenPriceAnalyticsCsv(
                    data,
