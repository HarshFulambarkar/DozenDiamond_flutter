...
class AnalysisAlphaGainVsOpenPriceRestApiService {
  ...
  Future<AnalysisAlphaGainVsOpenPriceResponse?> analyticsAlphaGainVsOpenPriceGraph(
      AlphaGainVsOpenPriceRequest stockData) async {
    try {
      ...
      Uri url = Uri.parse(
          "$baseUrl_v2$userId/analytics/alphaGain-vs-price");
      ...
      if (httpStatusChecker(response)) {
        ...
        return AnalysisAlphaGainVsOpenPriceResponse.fromJson(apiResponse);
      } else {
  ...
  Future<CsvDownloadResponse?> alphaGainVsOpenPriceAnalyticsCsv(
      stockData) async {
    print("inside alphaGainVsOpenPriceAnalyticsCsv");
    print(stockData);
    try {
      print("inside try");
      ...
      var url = Uri.parse(
          "$baseUrl_v2$userId/analytics/alphaGain-vs-price/downloadCsv");
