...
class AnalysisStockValuePriceScreen extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisStockValuePriceScreen(
      {super.key,
        required this.userSelectedAnalysis,
        required this.userSelectedStock});

  @override
  State<AnalysisStockValuePriceScreen> createState() => _AnalysisStockValuePriceScreenState();
}

class _AnalysisStockValuePriceScreenState extends State<AnalysisStockValuePriceScreen> {
  ...
  Widget stockValueVsPriceCsvDialog() {
    return Container(
      child: TextButton(
        onPressed: () {
  ...
  Future<void> getPriceVsValueGraphData() async {
    print("inside getPriceVsValueGraphData");
    ...
    try {
      ...
      if (graphData!.status!) {
        ...
        for (var entry in graphData!.data!) {
          if (!(entry.type == "Cumulative")) {
            ...
            print(entry.units);
            print(entry.price);
            print("the graphData is here value $value");

            hiddenGraphData.add(FlSpot(entry.price ?? 0.0, value));
      ...
      print("getPriceVsValueGraphData function $err");
                ...
                for (int i = 0; i < value.length; i++) {
                  if (i == value.length - 1) {
                    ...
                    tooltipData.add(LineTooltipItem(
                        // "${i == 0 ? 'Price: ${value[i].x}\n' : ''}Stock Value: ${value[i].y.toStringAsFixed(2)}",
                        "Stock Value: ${value[i].y.toStringAsFixed(2)}\n${i == 0 ? 'Price: ${value[i].x}' : ''}",
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        //   ),
                        // ),
                        _selectedAnalysisType!.itemId == 3 || //1 ||
