    ...
    try {
      for (var i = 0; i < graphData.orders!.length; i++) {
        hiddenGraphData.add(FlSpot(
            double.parse((graphData.orders![i].totalUnits!).toString()),
            double.tryParse((graphData.orders![i].value).toString()) ?? 0));
        selectedGraphData.add(LineChartBarData(
          spots: [
            FlSpot(double.parse((graphData.orders![i].totalUnits!).toString()),
                double.tryParse((graphData.orders![i].value).toString()) ?? 0)
          ],
          barWidth: 10,
          color: Colors.white,
                ...
                for (int i = 0; i < value.length; i++) {
                  if (i == value.length - 1) {
                    ...
                    tooltipData.add(LineTooltipItem(
                        "${i == 0 ? 'tradeValueAtTime: ${value[i].x}\n' : ''}units: ${value[i].y}",
