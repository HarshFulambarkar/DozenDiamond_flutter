...
class AnalysisPriceVsNumberOfStocks extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisPriceVsNumberOfStocks(
      {super.key,
      required this.userSelectedAnalysis,
      required this.userSelectedStock});

  @override
  State<AnalysisPriceVsNumberOfStocks> createState() =>
      _AnalysisPriceVsNumberOfStocksState();
}

class _AnalysisPriceVsNumberOfStocksState
    extends State<AnalysisPriceVsNumberOfStocks> {
  ...
  void initState() {
    ...
    _endDateTime = _dateTimeState!.endFullDate;
    _startDateTime = _dateTimeState!.startFullDate;
    print(
        "here is the end Date $_endDateTime and the start date as $_startDateTime");
    endMonth = DateFormat('MMM').format(_endDateTime!);
    startMonth = DateFormat('MMM').format(_startDateTime!);
    ...
    try {
      for (var i = 0; i < graphData.orders!.length; i++) {
        hiddenGraphData.add(FlSpot(
            graphData.orders![i].executionPrice!,
            double.tryParse((graphData.orders![i].totalUnits).toString()) ??
                0));
      ...
      print("error in the getProfitVsPriceGraphData $err");
    }
                                  ...
                                  children: [
                                    Container(
                                        ...
                                        margin: EdgeInsets.only(
                                            ...
                                            top: 5,
                                            bottom: 10),
                                        child: selectedGraphData.length <= 1
                                            ? Container(
                                                child: Text(
                                                    "No Data to plot a graph",
                                                    style: TextStyle(
                                                        color: Colors.yellow)),
                                              )
                                            : profitVsPriceLineChart()),
                                    selectedGraphData.length <= 2
                                        ? Container()
                                        : Container(
