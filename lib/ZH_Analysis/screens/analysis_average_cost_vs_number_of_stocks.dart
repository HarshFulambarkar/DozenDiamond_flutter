...
class AnalysisAverageCostVsNumberOfStocksScreen extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisAverageCostVsNumberOfStocksScreen(
      {super.key,
        required this.userSelectedAnalysis,
        required this.userSelectedStock});

  @override
  State<AnalysisAverageCostVsNumberOfStocksScreen> createState() => _AnalysisAverageCostVsNumberOfStocksScreenState();
}

class _AnalysisAverageCostVsNumberOfStocksScreenState extends State<AnalysisAverageCostVsNumberOfStocksScreen> {
  String? warningMessage = "";
  String? _selectedStock;
  TradeAnalyticsTimeDropdown? _selectedTimePeriod;
  AnalysisType? _selectedAnalysisType;
  DateTimeProvider? _dateTimeState;
  AverageCostVsPriceNumberOfStocksResponse? graphData;
  ...
  void initState() {
    ...
    getAverageCostVsNumberOfStocksData();
  ...
  Widget averageCostVsNumberOfStocksCsvDialog() {
    return Container(
      child: TextButton(
        onPressed: () {
          // showDialog(
  ...
  Future<void> getAverageCostVsNumberOfStocksData() async {
    print("inside getAverageCostVsNumberOfStocksData");
    loadingGraphData = true;
    try {
      graphData = await AnalyticsRestApiService()
          .analyticsAverageCostVsPriceNumberOfStocksGraph(GraphDataRequest(
      ...
      if (graphData!.status!) {
        ...
        for (var entry in graphData!.data!) {
          ...
          double averageCost = entry.averageCost!.toDouble();
          double numberOfStocks = entry.numberOfStocks!.toDouble();
          // print("------------------------");
          ...
          hiddenGraphData.add(FlSpot(numberOfStocks, averageCost, ));
      ...
      LineChartData(
                ...
                for (int i = 0; i < value.length; i++) {
                  if (i == value.length - 1) {
                    ...
                    tooltipData.add(LineTooltipItem(
                        "Average Cost: ${value[i].y.toStringAsFixed(2)}\n${i == 0 ? 'Number of Stocks: ${value[i].x}\n' : ''}",
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            // axisNameWidget: Text("Number of Stocks"),
            axisNameWidget: Text("Average Cost"),
            axisNameSize: 16,
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 50,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _selectedAnalysisType!.itemId == 4 || //1 ||
                            _selectedAnalysisType!.itemId == 7
                            ? !_graphDataAvailable
                            ? (loadingGraphData)
                            ?Container(
                          ...
                          children: [
                            averageCostVsNumberOfStocksCsvDialog(),
                            Container(
                                height:
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  AnalyticsRestApiService()
                      .stockAverageCostVsNumberOfStocksAnalyticsCsv(
                      data
