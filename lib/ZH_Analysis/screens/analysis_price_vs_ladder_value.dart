...
class AnalysisPriceVsLadderValueScreen extends StatefulWidget {
  final AnalysisType userSelectedAnalysis;
  final String userSelectedStock;
  const AnalysisPriceVsLadderValueScreen(
      {super.key,
        required this.userSelectedAnalysis,
        required this.userSelectedStock});

  @override
  State<AnalysisPriceVsLadderValueScreen> createState() => _AnalysisPriceVsLadderValueScreenState();
}

class _AnalysisPriceVsLadderValueScreenState extends State<AnalysisPriceVsLadderValueScreen> {
  String? warningMessage = "";
  String? _selectedStock;
  TradeAnalyticsTimeDropdown? _selectedTimePeriod;
  AnalysisType? _selectedAnalysisType;
  DateTimeProvider? _dateTimeState;
  PriceVsLadderValueResponse? graphData;
  ...
  Future<void> getPriceVsLadderValueData() async {
    print("inside getPriceVsLadderValueData");
    loadingGraphData = true;
    try {
      graphData = await AnalyticsRestApiService()
          .analyticsPriceVsLadderValueGraph(PriceVsLadderValueRequest(
      ...
      if (graphData!.status!) {
        ...
        for (var entry in graphData!.data!) {
          ...
          double laddersValue = (double.parse(entry.laddersValue!.toStringAsFixed(2)));
          // print("------------------------");
          ...
          // hiddenGraphData.add(FlSpot(laddersValue, price));
          hiddenGraphData.add(FlSpot(price, laddersValue));
      ...
      LineChartData(
                ...
                for (int i = 0; i < value.length; i++) {
                  if (i == value.length - 1) {
                    ...
                    tooltipData.add(LineTooltipItem(
                        "Ladders Value: ${value[i].y.toStringAsFixed(2)}\n${i == 0 ? 'Price: ${value[i].x}\n' : ''}",
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            ...
            axisNameWidget: Text("Ladders Value"),
            axisNameSize: 16,
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 50,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _selectedAnalysisType!.itemId == 4 || //1 ||
                            _selectedAnalysisType!.itemId == 17
                            ? !_graphDataAvailable
                            ? (loadingGraphData)
                            ?Container(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  AnalyticsRestApiService()
                      .stockLaddersValueVsPriceAnalyticsCsv(
                      data
